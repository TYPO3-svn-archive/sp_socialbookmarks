<?php
	...
	***************************************************************/
	...
	class tx_spsocialbookmarks_modfunc1_chart {
		 ...
		 */
		public function sGetChart ($paConfig, $psType='services', $psLabel='', $paBars=array(), $paImages=array()) {
			if (!is_array($paConfig) || !count($paConfig)) {
				return '';
			}
			...
			// Add bars and images
			if (is_array($paBars) && count($paBars)) {
				// Get 100%
				...
				foreach ($paBars as $sKey => $iCount) {
					...
					// Add image
					if ($sImage) {
						$this->sContent .= '<div style="'.$sWrapStyle.' left:'.$iLastLeft.'px; width:'.($iBarWidth + 3).'px;"><img src="'.$sImage.'" style="'.$sImageStyle.'" height="'.$iImageHeight.'" width="'.$iImageWidth.'" alt="'.$sAlt.'" title="'.$sTitle.'" /></div>';
					}

					// Add bar
					$this->sContent  .= '<div style="'.$sBarStyle.' width:'.$iBarWidth.'px; height:'.$iBarHeight.'px; left:'.$iLastLeft.'px; background:'.$sBarColor.';" title="'.$sTitle.'"></div>';
		 ...
		 *
		 * @return String with complete style information
		 */
		protected function sGetStyle($psType) {
			...
			if (is_array($aStyle) && count($aStyle)) {
	...
	if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sp_socialbookmarks/modfunc1/class.tx_spsocialbookmarks_modfunc1_chart.php'])	{
		include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sp_socialbookmarks/modfunc1/class.tx_spsocialbookmarks_modfunc1_chart.php']);
	}
